...
export class WeatherService implements OnDestroy {
  ...
  getCityWeatherByQuery(query: string): Observable<CityWeather> {//fazendo requisição por query
  ...
  getCityWeatherById(id: string): Observable<CityWeather> { //fazendo requisição por ID
    const params = new HttpParams({fromObject: {id}});
    return this.doGet<any>('weather', params)
      .pipe(map(response => responseToCityWeather(response)));
  }

  getCityWeatherByCoord(lat: number, lon: number): Observable<CityWeather> { //fazendo requisição pelas coordenadas
  ...
  getWeatherDetails(lat: number, lon: number): Observable<CityDailyWeather> { //retornar detalhes do clima
    const params = new HttpParams({fromObject: {
      lat: lat.toString(),
      lon: lon.toString(),
